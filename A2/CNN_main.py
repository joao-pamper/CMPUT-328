...
def should_save_model(
    ...
    if criteria not in ["last", "loss", "accuracy"]:
        raise ValueError(
            "Invalid save criteria. Choose either 'loss', 'accuracy', or 'last'"
        )
    ...
    if criteria == "last" and epoch == num_epochs - 1:
        ...
        print(
            f"Validation loss decreased ({best_val_loss:.4f} --> {val_loss:.4f}). Saving model..."
        )
        return True
    elif criteria == "accuracy" and val_accuracy > best_val_accuracy:
        print(
            f"Validation accuracy increased ({best_val_accuracy:.4f} --> {val_accuracy:.4f}). Saving model..."
        )
        return True
...
def test(model, device, test_transform):
...
def CNN(pretrain, device, load_model):
    ...
    accuracy = test(model, device, test_transforms)
    ...
    result = OrderedDict(accuracy=accuracy, score=score, run_time=run_time)
...
def main():
    ...
    try:
        ...
        print(
            "WARNING: You have not installed paramparse. Please manually edit the arguments."
        )
